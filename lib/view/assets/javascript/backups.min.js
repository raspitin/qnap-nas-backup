...
var Restore = function Restore() {
  ...
  model.setStatus({
    type: 'pro',
    message: qnap_locale.restore_from_file
  });
};
...
Import.prototype.start = function (options, retries) {
  ...
  $(window).bind('beforeunload', function () {
    return qnap_locale.stop_importing_your_website;
  }); // Set initial status

  this.setStatus({
    type: 'info',
    message: qnap_locale.preparing_to_import
  }); // Set params

  var params = this.params.concat({
    name: 'secret_key',
    value: qnap_import.secret_key
  }); // Set additional params
  ...
  $.ajax({
    ...
    try {
      ...
      if (json) {
        ...
        if (error.message) {
          ...
          self.setStatus({
            type: 'error',
            title: qnap_locale.unable_to_import,
            message: error.message
          });
          return;
        }
    ...
    if (retries >= 5) {
      ...
      self.setStatus({
        type: 'error',
        title: qnap_locale.unable_to_import,
        message: qnap_locale.unable_to_start_the_import
      });
      return;
    }
  ...
  $.ajax({
    ...
    if (retries >= 5) {
      ...
      self.setStatus({
        type: 'error',
        title: qnap_locale.unable_to_import,
        message: qnap_locale.unable_to_confirm_the_import
      });
      return;
    }
...
Import.prototype.checkDiskSpace = function (fileSize, callback) {
  ...
  var diskSpaceFree = parseInt(qnap_disk_space.free, 10);
  var diskSpaceFactor = parseInt(qnap_disk_space.factor, 10);
  var diskSpaceExtra = parseInt(qnap_disk_space.extra, 10);
  ...
  if (diskSpaceFree >= 0) {
    ...
    if (diskSpaceRequired > diskSpaceFree) {
      this.setStatus({
        ...
        message: qnap_locale.out_of_disk_space.replace('%s', QNAP.Util.sizeFormat(diskSpaceRequired - diskSpaceFree))
      });
      return;
    }
  }
  ...
  $.ajax({
    ...
    if (retries >= 5) {
      ...
      self.setStatus({
        type: 'error',
        title: qnap_locale.unable_to_import,
        message: qnap_locale.unable_to_prepare_blogs_on_import
      });
      return;
    }
...
Import.prototype.clean = function (options, retries) {
  ...
  this.setStatus({
    type: 'info',
    message: qnap_locale.please_wait_stopping_the_import
  }); // Set params
  ...
  $.ajax({
    ...
    if (retries >= 5) {
      ...
      self.setStatus({
        type: 'error',
        title: qnap_locale.unable_to_import,
        message: qnap_locale.unable_to_stop_the_import
      });
      return;
    }
...
var Modal = function Modal() {
  ...
  this.error = function (params) {
    ...
    closeButton.append(qnap_locale.close_import); // Append close button to action
  ...
  this.progress = function (params) {
    ...
    var progressBar = $('<span class="qnap-progress-bar"></span>'); // Create progress bar meter

    this.progress.progressBarMeter = $('<span class="qnap-progress-bar-meter"></span>').width(params.percent + '%'); // Create progress bar percent

    this.progress.progressBarPercent = $('<span class="qnap-progress-bar-percent"></span>').text(params.percent + '%'); // Create stop import
    ...
    stopButton.append('<i class="qnap-icon-notification"></i> ' + qnap_locale.stop_import); // Append progress meter and progress percent
  ...
  this.pro = function (params) {
    ...
    var message = $('<p class="qnap-import-modal-content"></p>').html(params.message); // Create action section

    var action = $('<div></div>'); // Create warning

    var warning = $('<i class="qnap-icon-notification"></i>'); // Create close button

    var closeButton = $('<button type="button" class="qnap-button-gray"></button>').on('click', function () {
      self.destroy();
    }); // Append text to close button
  ...
  this.confirm = function (params) {
    ...
    var action = $('<div class="qnap-import-modal-actions"></div>'); // Create warning
    ...
    var confirmButton = $('<button type="button" class="qnap-button-green"></button>').on('click', function () {
      confirmButton.attr('disabled', 'disabled');
      self.onConfirm();
    }); // Append text to close button

    closeButton.append(qnap_locale.close_import); // Append text to confirm button

    confirmButton.append(qnap_locale.confirm_import + ' &gt;'); // Append close button to action
  ...
  this.diskSpaceConfirm = function (params) {
    ...
    confirmButton.append(qnap_locale.confirm_disk_space); // Append close button to action
  ...
  this.blogs = function (params) {
    ...
    var title = $('<span></span>').addClass('qnap-title-grey').text(params.title); // Create continue button

    var continueButton = $('<button type="submit" class="qnap-button-green"></button>'); // Append text to continue button

    continueButton.append(qnap_locale.continue_import); // Append continue button to action
  ...
  this.info = function (params) {
    ...
    var loader = $('<span class="qnap-loader"></span>'); // Create warning

    var warning = $('<p></p>').html(qnap_locale.please_do_not_close_this_browser); // Create notice to be displayed during import process

    var notice = $('<div class="qnap-import-modal-notice"></div>'); // Append warning to notice
  ...
  this.done = function (params) {
    ...
    var message = $('<p class="qnap-import-modal-content-done"></p>').html(params.message); // Create action section

    var action = $('<div class="qnap-import-modal-actions"></div>'); // Create title

    var title = $('<span></span>').addClass('qnap-title-green').text(params.title); // Create close button

    var closeButton = $('<button type="button" class="qnap-button-green"></button>').on('click', function () {
      self.destroy();
    }); // Append text to close button

    closeButton.append(qnap_locale.finish_import + ' &gt;'); // Append close button to action
...
Modal.prototype.render = function (params) {
  $(document).trigger('qnap-import-status', params); // Show modal
...
jQuery(document).ready(function ($) {
  ...
  $('#qnap-backups-list').on({
    mouseenter: function mouseenter() {
      $(this).addClass('qnap-button-on');
    },
    mouseleave: function mouseleave() {
      $(this).removeClass('qnap-button-on');
    }
  }, '.qnap-backup-actions > a'); // Delete file

  $('#qnap-backups-list').on('click', '.qnap-backup-delete', function (e) {
    var self = $(this); // Delete file

    /* eslint-disable no-alert */

    if (confirm(qnap_locale.want_to_delete_this_file)) {
      ...
      $.ajax({
        ...
        if (data.errors.length === 0) {
          ...
          if ($('.qnap-backups tbody tr').length === 1) {
            $('.qnap-backups').hide();
            $('.qnap-backups-empty').show();
          }
        }
      });
    }
  ...
  $('#qnap-backups-list').on('click', '.qnap-backup-restore', function (e) {
    e.preventDefault();
    /* eslint-disable no-unused-vars */

    if (QNAP.MultisiteExtensionRestore) {
      var restore = new QNAP.MultisiteExtensionRestore($(this).data('archive'), $(this).data('size'));
    } else if (QNAP.UnlimitedExtensionRestore) {
      var _restore = new QNAP.UnlimitedExtensionRestore($(this).data('archive'), $(this).data('size'));
    } else if (QNAP.FreeExtensionRestore) {
      var _restore2 = new QNAP.FreeExtensionRestore($(this).data('archive'), $(this).data('size'));
    } else {
      var _restore3 = new QNAP.Restore($(this).data('archive'), $(this).data('size'));
    }
    /* eslint-enable no-unused-vars */

  });
  $('#qnap-backups-list').on('click', '.qnap-backup-label-description, .qnap-backup-label-text', function () {
    $(this).hide();
    $(this).closest('.qnap-column-name').find('.qnap-backup-label-holder').show();
    $(this).closest('.qnap-column-name').find('.qnap-backup-label-field').trigger('focus');
  });
  $('#qnap-backups-list').on('keydown', '.qnap-backup-label-field', function (e) {
    var self = $(this);
    ...
    if (e.which === 13) {
      e.preventDefault();
      self.hide();
      self.closest('.qnap-backup-label-holder').append(spinner);
      $.ajax({
        url: qnap_backups.labels.url,
        type: 'POST',
        dataType: 'json',
        data: {
          secret_key: qnap_backups.secret_key,
        ...
        if (data.errors.length === 0) {
          ...
          if (self.val()) {
            self.closest('.qnap-backup-label-holder').hide();
            self.closest('.qnap-column-name').find('.qnap-backup-label-text').show();
            self.closest('.qnap-column-name').find('.qnap-backup-label-colored').text(self.val());
          } else {
            self.closest('.qnap-backup-label-holder').hide();
            self.closest('.qnap-column-name').find('.qnap-backup-label-description').removeClass('qnap-backup-label-selected').removeAttr('style');
          }
      ...
      if (self.data('value')) {
        ...
        self.closest('.qnap-backup-label-holder').hide();
        self.closest('.qnap-column-name').find('.qnap-backup-label-text').hide();
        self.closest('.qnap-column-name').find('.qnap-backup-label-description').removeClass('qnap-backup-label-selected').removeAttr('style');
      }
  ...
  $(document).on('qnap-export-status', function (e, params) {
    if (params.type === 'download') {
      if ($('.qnap-backups tbody tr').length > 1) {
        $('.qnap-backups-list-spinner-holder').show();
      } else {
        $('.qnap-backups-empty').hide();
        $('.qnap-backups-empty-spinner-holder').show();
      }

      $.get(qnap_backups.backups.url, {
        secret_key: qnap_backups.secret_key
      }).done(function (data) {
        $('#qnap-backups-create').find('.qnap-backups-empty').hide();
        $('#qnap-backups-create').find('.qnap-backups-empty-spinner-holder').hide();
        $('#qnap-backups-list').html(data);
      });
    }
  });
  model = new Export();
