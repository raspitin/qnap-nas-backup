...
FileUploader.prototype.init = function () {
  ...
  var formElement = $('#qnap-import-form');
  var selectElement = $('#qnap-import-file');
  var dropElement = $('#qnap-drag-drop-area');
  selectElement.on('change', function (e) {
    ...
    if (file) {
      ...
      if (_this.fileSize > qnap_uploader.max_file_size) {
        _this.model.setStatus({
          type: 'pro',
          message: qnap_locale.import_from_file
        });
      } else {
  ...
  dropElement.on('dragenter', function (e) {
    dropElement.addClass('qnap-drag-over');
    e.preventDefault();
  });
  ...
  dropElement.on('dragleave', function (e) {
    dropElement.removeClass('qnap-drag-over');
    e.preventDefault();
  });
...
FileUploader.prototype.c1 = function (file) {
  if (file.name.substr(-6) !== 'qwp') {
    throw new Error(qnap_locale.invalid_archive_extension);
  }
}; // Check compatibility


FileUploader.prototype.c3 = function () {
  if (qnap_compatibility.messages.length > 0) {
    throw new Error(qnap_compatibility.messages.join());
  }
};
...
FileUploader.prototype.onBeforeUpload = function (file) {
  ...
  var options = QNAP.Util.form('#qnap-import-form').concat({
    name: 'storage',
    value: storage
  }).concat({
    name: 'archive',
...
FileUploader.prototype.upload = function (file) {
  ...
  for (var name in qnap_uploader.params) {
    formData.append(name, qnap_uploader.params[name]);
  }

  $.ajax({
    url: qnap_uploader.url,
    type: 'POST',
    data: formData,
    cache: false,
    contentType: false,
...
jQuery(document).ready(function ($) {
  ...
  if (QNAP.MultisiteExtensionUploader) {
    uploader = new QNAP.MultisiteExtensionUploader();
  } else if (QNAP.UnlimitedExtensionUploader) {
    uploader = new QNAP.UnlimitedExtensionUploader();
  } else if (QNAP.FileExtensionUploader) {
    uploader = new QNAP.FileExtensionUploader();
  } else {
    uploader = new QNAP.FileUploader();
  }

  uploader.init(); // Expands/Collapses Import from

  $('.qnap-expandable > div.qnap-button-main').on('click', function () {
