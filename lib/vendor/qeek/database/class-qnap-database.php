...
abstract class QNAP_Database {
	...
	public function __construct( $wpdb ) {
		...
		if ( is_resource( $this->wpdb->dbh ) ) {
			if ( get_resource_type( $this->wpdb->dbh ) === 'SQL Server Connection' ) {
				throw new QNAP_Database_Exception(
					__(
						'Your WordPress installation uses Microsoft SQL Server. ' .
						'To use QNAP WP Migration, please change your installation to MySQL and try again. ' .
						'<a href="https://help.qeek.com/knowledgebase/microsoft-sql-server/" target="_blank">Technical details</a>',
						QNAP_PLUGIN_NAME
					),
					501
				);
	 ...
	 * @return boolean
	 */
	public function export( $file_name, &$query_offset = 0, &$table_index = 0, &$table_offset = 0, &$table_rows = 0, &$table_rows_count = 0 ) {
		// Set file handler
		$file_handler = qnap_open( $file_name, 'cb' );
		...
		if ( fseek( $file_handler, $query_offset ) !== -1 ) {
			...
			// Write headers
			if ( $query_offset === 0 ) {
				qnap_write( $file_handler, $this->get_header() );
			}
			...
			for ( ; $table_index < count( $tables ); ) {
				...
				if ( in_array( $table_name, $views ) ) {
					...
					if ( $table_offset === 0 ) {
						...
						qnap_write( $file_handler, $drop_view );
						...
						qnap_write( $file_handler, $create_view );

						// Write end of statement
						qnap_write( $file_handler, ";\n\n" );
					}
					...
					if ( $table_offset === 0 ) {
						...
						qnap_write( $file_handler, $drop_table );
						...
						qnap_write( $file_handler, $create_table );
					...
					do {
						...
						if ( $primary_keys ) {
							...
							$query = sprintf( 'SELECT t1.* FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d) AS t2 USING (%s)', $table_name, $table_keys, $table_name, $table_where, $table_keys, $table_offset, QNAP_MAX_SELECT_RECORDS, $table_keys );
							$query_count = sprintf( 'SELECT count(*) as total FROM (SELECT t1.* FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s) AS t2 USING (%s)) t', $table_name, $table_keys, $table_name, $table_where, $table_keys, $table_keys );
							...
							// Set query with offset and rows count
							$query = sprintf( 'SELECT * FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d', $table_name, $table_where, $table_keys, $table_offset, QNAP_MAX_SELECT_RECORDS );
							$query_count = sprintf( 'SELECT count(*) as total FROM `%s` WHERE %s', $table_name, $table_where );
						}
						...
						if ( $num_rows = $this->num_rows( $result ) ) {
							...
							while ( $row = $this->fetch_assoc( $result ) ) {
								...
								if ( $table_offset % QNAP_MAX_TRANSACTION_QUERIES === 0 ) {
									qnap_write( $file_handler, "START TRANSACTION;\n" );
								}
								...
								qnap_write( $file_handler, $table_insert );
								...
								if ( $table_offset % QNAP_MAX_TRANSACTION_QUERIES === 0 ) {
									qnap_write( $file_handler, "COMMIT;\n" );
								}
							}
						} else {
						...
						if ( ( $timeout = apply_filters( 'qnap_completed_timeout', 10 ) ) ) {
							if ( ( microtime( true ) - $start ) > $timeout ) {
								$result_count = $this->query( $query_count );
								$row = $this->fetch_assoc( $result_count );
								$table_rows_count = (int)$row['total'];
								$completed = false;
								break 2;
							}
						}
	...
	public function import( $file_name, &$query_offset = 0 ) {
		...
		// Set file handler
		$file_handler = qnap_open( $file_name, 'rb' );
				...
				if ( preg_match( '/;\s*$/S', $query ) ) {
					...
					if ( strlen( $query ) <= $max_allowed_packet ) {
						...
						if ( ! $this->is_cache_query( $query ) ) {
							...
							if ( $this->errno() === 1813 ) {
								throw new QNAP_Database_Exception( __( 'Error importing database table.', QNAP_PLUGIN_NAME ), 503 );
							}
								...
								if ( stripos( $this->error(), 'max_queries_per_hour' ) !== false ) {
									throw new QNAP_Database_Exception(
										__(
											...
											'To use QNAP WP Migration, please increase MySQL max_queries_per_hour limit. ' .
											'<a href="https://help.qeek.com/knowledgebase/mysql-error-codes/#max-queries-per-hour" target="_blank">Technical details</a>',
											QNAP_PLUGIN_NAME
										),
										503
									);
									...
									throw new QNAP_Database_Exception(
										__(
											...
											'To use QNAP WP Migration, please increase MySQL max_updates_per_hour limit. ' .
											'<a href="https://help.qeek.com/knowledgebase/mysql-error-codes/#max-updates-per-hour" target="_blank">Technical details</a>',
											QNAP_PLUGIN_NAME
										),
										503
									);
									...
									throw new QNAP_Database_Exception(
										__(
											...
											'To use QNAP WP Migration, please increase MySQL max_connections_per_hour limit. ' .
											'<a href="https://help.qeek.com/knowledgebase/mysql-error-codes/#max-connections-per-hour" target="_blank">Technical details</a>',
											QNAP_PLUGIN_NAME
										),
										503
									);
									...
									throw new QNAP_Database_Exception(
										__(
											...
											'To use QNAP WP Migration, please increase MySQL max_user_connections limit. ' .
											'<a href="https://help.qeek.com/knowledgebase/mysql-error-codes/#max-user-connections" target="_blank">Technical details</a>',
											QNAP_PLUGIN_NAME
										),
										503
									);
	...
	protected function replace_visual_composer_values_callback( $matches ) {
		...
		if ( QNAP_Database_Utility::base64_validate( $matches[1] ) ) {

			// Decode base64 characters
			$matches[1] = QNAP_Database_Utility::base64_decode( $matches[1] );

			// Replace values
			$matches[1] = QNAP_Database_Utility::replace_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );

			// Encode base64 characters
			$matches[1] = QNAP_Database_Utility::base64_encode( $matches[1] );
		}
	...
	protected function replace_oxygen_builder_values_callback( $matches ) {
		// Validate base64 data
		if ( QNAP_Database_Utility::base64_validate( $matches[2] ) ) {

			// Decode base64 characters
			$matches[2] = QNAP_Database_Utility::base64_decode( $matches[2] );

			// Replace values
			$matches[2] = QNAP_Database_Utility::replace_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[2] );

			// Encode base64 characters
			$matches[2] = QNAP_Database_Utility::base64_encode( $matches[2] );
		}
	...
	protected function replace_base64_values_callback( $matches ) {
		...
		if ( QNAP_Database_Utility::base64_validate( $matches[1] ) ) {
			...
			// Replace serialized values
			$matches[1] = QNAP_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );
	...
	protected function replace_table_values_callback( $matches ) {
		...
		$matches[1] = QNAP_Database_Utility::unescape_mysql( $matches[1] );

		// Replace serialized values
		$matches[1] = QNAP_Database_Utility::replace_serialized_values( $this->get_old_replace_values(), $this->get_new_replace_values(), $matches[1] );

		// Escape MySQL special characters
		$matches[1] = QNAP_Database_Utility::escape_mysql( $matches[1] );
	...
	protected function replace_raw_values( $input ) {
		return QNAP_Database_Utility::replace_values( $this->get_old_replace_raw_values(), $this->get_new_replace_raw_values(), $input );
	}
